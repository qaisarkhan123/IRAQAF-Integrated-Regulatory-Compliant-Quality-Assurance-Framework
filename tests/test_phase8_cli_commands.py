...
class TestSystemCommands:
    ...
    def test_list_systems_command(self, runner):
        ...
        assert result.exit_code in [0, 1]  # May succeed or fail depending on API
    ...
    def test_create_system_command(self, runner):
        ...
        # Simulate user input
        result = runner.invoke(cli, ['create-system'], input='Test System\nTest medical device\nmedical_device\n')
...
class TestRegulatoryCommands:
    ...
    def test_search_requirements_command(self, runner):
        ...
        result = runner.invoke(cli, ['search-requirements', '--keyword', 'encryption'])
        assert result.exit_code in [0, 1]
    
    @pytest.mark.skipif(not CLI_AVAILABLE, reason="CLI module not available")
    def test_search_with_filters(self, runner):
        """Test search with severity filter"""
        result = runner.invoke(cli, ['search-requirements', '--severity', 'high'])
        assert result.exit_code in [0, 1]
...
class TestCLIErrorHandling:
    ...
    def test_invalid_filter_value(self, runner):
        ...
        result = runner.invoke(cli, ['list-changes', '--severity', 'invalid-severity'])
