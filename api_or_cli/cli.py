...
def make_request(method: str, endpoint: str, data: Optional[dict] = None, params: Optional[dict] = None):
    ...
    try:
        ...
        click.echo("Error: Could not connect to API. Is it running on port 8000?", err=True)
        raise SystemExit(1)
...
def delete_system(system_id):
    ...
    if click.confirm(f"Are you sure you want to delete system '{system_id}'?"):
        click.echo(click.style(f"🗑️  Deleting system '{system_id}'...", fg="red"))
...
def assess(system_id):
    ...
    click.echo(click.style(f"📋 Running assessment for system '{system_id}'...", fg="cyan"))
    ...
    result = make_request("POST", f"/systems/{system_id}/assess", assessment_data)
...
def generate_report(system_id, format):
    ...
    click.echo(click.style(f"📄 Generating report for '{system_id}'...", fg="cyan"))
    ...
    # Export
    if format == "json":
        export_result = make_request("GET", f"/reports/{system_id}/export", params={"format": "json"})
...
def import_data(filename):
    ...
    try:
        ...
        for system_data in systems:
            ...
            click.echo(click.style(f"✓ Imported: {system_data['id']}", fg="green"))
...
def export_results(system_id, output):
    ...
    click.echo(click.style(f"📤 Exporting results for '{system_id}'...", fg="cyan"))
    
    result = make_request("GET", f"/reports/{system_id}/export", params={"format": "json"})
