...
class GapAnalyzer:
        ...
        for req_id, score_obj in requirement_scores.items():
            if score_obj.compliance_score < gap_threshold:
                ...
                severity = self._determine_severity(
                    gap_size, score_obj.risk_level.value)
                ...
                logger.info(
                    f"Identified gap {gap_id}: {severity.name} severity")
    ...
    def _analyze_root_cause(self, score_obj) -> str:
        ...
        avg_quality = statistics.mean(
            [e.quality_score for e in score_obj.evidence_list])
        if avg_quality < 60:
    ...
    def generate_remediation_plan(
        ...
        if gap.severity == GapSeverity.CRITICAL:
            ...
            actions.extend([
                self._create_action(
                    "RAC-01", RemediationType.DOCUMENTATION, gap),
                self._create_action(
                    "RAC-02", RemediationType.POLICY_CREATION, gap),
                self._create_action(
                    "RAC-03", RemediationType.IMPLEMENTATION, gap)
            ])
        elif gap.severity == GapSeverity.HIGH:
            actions.extend([
                self._create_action(
                    "RAH-01", RemediationType.DOCUMENTATION, gap),
                self._create_action(
                    "RAH-02", RemediationType.IMPLEMENTATION, gap)
            ])
        elif gap.severity == GapSeverity.MEDIUM:
            actions.append(
                self._create_action(
                    "RAM-01", RemediationType.DOCUMENTATION, gap)
            )
        else:  # LOW
            actions.append(
        ...
        logger.info(
            f"Generated {len(actions)} remediation actions for {gap.gap_id}")
    ...
    def get_portfolio_gap_summary(self) -> Dict:
        ...
        max_timeline = max(
            (max(a.timeline_days for a in g.remediation_actions)
             if g.remediation_actions else 0)
            for g in gaps_list
        )
    ...
    def _calculate_action_priority(self, gap: ComplianceGap, action: RemediationAction) -> float:
        ...
        timeline_score = (
            100 - action.timeline_days) if action.timeline_days <= 100 else 0

        # Lower cost might enable quicker action
        cost_score = (1000 - action.estimated_cost) / \
            10 if action.estimated_cost <= 1000 else 0
...
if __name__ == "__main__":
    ...
    for gap in gaps:
        ...
        for action in actions:
            print(
                f"    - {action.action_type.value}: {action.estimated_hours}h, ${action.estimated_cost}")
