...
class ComplianceScorer:
    ...
    def score_requirement(
        ...
        if evidence_list:
            ...
            confidence = statistics.stdev(weighted_evidence) if len(
                weighted_evidence) > 1 else 0
        ...
        if evidence_list:
            std_dev = statistics.stdev([e.quality_score for e in evidence_list]) if len(
                evidence_list) > 1 else 0
        ...
        logger.info(
            f"Scored {requirement_id}: {compliance_score:.1f}% ({compliance_level.name})")
    ...
    def get_portfolio_summary(self) -> Dict:
        ...
        for level in ComplianceLevel:
            ...
            level_dist[level.name] = {"count": count,
        ...
        for risk in RiskLevel:
            ...
            percentage = (count / len(all_scores)) * 100
            risk_dist[risk.name] = {"count": count,
                                    "percentage": round(percentage, 1)}
...
class EvidenceMatrix:
    ...
    def add_evidence(
        ...
        logger.debug(
            f"Added {evidence.type.value} evidence for {requirement_id}")
