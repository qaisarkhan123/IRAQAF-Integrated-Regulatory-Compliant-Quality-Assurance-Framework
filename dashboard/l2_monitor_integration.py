        ...
        if st.button("▶️ Start Full Scan", use_container_width=True):
            with st.spinner("Scanning system..."):
                try:
                    ...
                    st.success(
                        f"✅ Scan Complete - Score: {scan.overall_score}/100")
...
def render_l2_metrics_dashboard():
    ...
    with col1:
        st.metric("🔐 Encryption",
                  f"{metrics_data['Encryption Coverage']}%", "+2.3%")

    with col2:
        st.metric("🔑 Access Control",
                  f"{metrics_data['Access Control']}%", "+1.8%")

    with col3:
        st.metric(
            "📝 Logging", f"{metrics_data['Logging & Monitoring']}%", "+0.5%")

    with col4:
        st.metric("💾 Data Protection",
                  f"{metrics_data['Data Protection']}%", "+1.2%")
...
def render_l2_alerts():
    ...
    alerts = [
        {'type': 'warning', 'message': '⚠️ Legacy TLS version detected on 3 endpoints',
            'time': '5 min ago'},
        {'type': 'info', 'message': 'ℹ️ Monthly compliance report ready for download',
            'time': '2 hours ago'},
        {'type': 'success', 'message': '✅ All security patches applied',
            'time': '1 day ago'},
    ]
